...
{ 
    public static String criticalBridges(int input1,String input2)
    {
        ...
        for(int i=0; i < testCases.length;i++){
        		...
        		if(connectedIcelands.length ==2){
        			...
        			for(int ic=0; ic < icelands.length; ic++){
        				if(connectedIcelands[0].equals(icelands[ic].getName())){
        					ic1 = icelands[ic];
        					match++;
        				}else if(connectedIcelands[1].equals(icelands[ic].getName())){
        					ic2 = icelands[ic];
        					match++;
        				}
        				if(match ==2)
        	...
        	for(int ic=0; ic < icelands.length; ic++){
        		...
        		if(iceland.getBrigeCount() == 1){
        			if(iceland.getBridges().get(0).isCritical())
        				continue;
        			...
        			result.append(iceland.getBridgesAsString());
        			iceland.getBridges().get(0).setCritical();
        			criticalBridgeCount++;
        		}else{
    ...
    private static class Bridge{
    	...
    	public String toString(){
    		return new StringBuilder().append('(').append(iceland1.getName()).append(',').append(iceland2.getName()).append(')').toString();
    	}
